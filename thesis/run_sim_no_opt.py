...
def runSims(assignonly=False,drone=False):
	...
	while i < 50000:
		...
		env.getEnv()
		greedy = Greedy(env) 
		sim2 = Simulation(greedy, drone=drone)
		revealed_grid, cost_greedy, total_u_greedy = sim2.move_until_fail()
		# print(revealed_grid)
		if assignonly and not sim2.assignments: #if we want to compare envs with assignments but get none move to next
			continue
		# print(revealed_grid)
		if revealed_grid is not None:
			...
			sellers = greedy.sellers
			buyers = greedy.buyers
		...
		if i % 100 == 99:
			with open('allugv-50k.txt', 'w') as f:
