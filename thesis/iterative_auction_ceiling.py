...
class IterativeAuctionCeiling: 
	...
	def iterative_market(self, eta, drone):
		...
		for _ in range(50):
			...
			total_cost = 0
			# Remove duplicate waypoint assignments
			...
			for p in surplus_pts:
				if p in excess_supply:
					newprice = max(0, self.waypt_prices[p] - eta)
					self.waypt_prices[p] = newprice
				elif p in excess_demand:
					newprice =  min(self.values[p], self.waypt_prices[p] + eta)
...
if __name__ == "__main__":
	...
	g= IterativeAuctionCeiling(env, [(4, 2), (3, 1)], [(0, 3), (4, 3)]) 
	# print(g.agents[2])
