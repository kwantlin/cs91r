...
class Simulation:
	def __init__(self, planner, revealed_grid=None, dijk_planner=None):
		self.planner = planner
		self.agents = planner.agents
		self.helper_paths = {}
		self.revealed_grid=revealed_grid
		self.dijk_planner = dijk_planner
	# def nofails(self):
	# 	for a self.agents
	...
	def move_until_fail_allugv(self):
		planner = self.planner
		if not self.dijk_planner:
			self.dijk_planner = planner
			...
			if waypts == []: #time to get nonhelpers' new paths
				if self.nonhelper_paths is None:
					...
					for b in self.nonhelpers:
						self.nonhelper_paths[b] = self.dijk_planner.dijkstra(grid, b, planner.dests[planner.agents.index(b)], planner.rewards[planner.agents.index(b)])[0]
...
def runSims(typeenv=None, assignonly=False):
	iter_auc_costs = []
	opt_costs = []
	nosell_costs = []
	...
	for i in range(10):
		print(i)
		env = EnvGenerator(8,8,8,0.3,0.3,0.4,64)
		env.getEnv()
		if typeenv is None:
	...
	# plt.savefig("cost-comp-drone.png")
	# plt.show() 
	

if __name__ == "__main__":
	runSims("drone", assignonly=True)
